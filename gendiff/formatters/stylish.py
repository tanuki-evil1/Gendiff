...
def stylish(diff_list: dict) -> str:  # Попробовать переписать под match case
    def walk(node, accumulator=1):
        ...
        for key, value in node.items():
            ...
            for index in range(
                    len(sign)):  # Из-за изменения структуры данных, пришлось ставить условие при котором sign
                if len(sign) == 2:  # Может быть не только + или -, а еще и -+, обыграли циклом и создали условие, т.к
                    if index > 0:  # значение теперь не только словарь или какой-то тип данных, но еще и кортеж с двумя
                        sub_value = normalize_value(value[1])  # значениями, показалась более удобной из-за того, что не надо думать о сортировке значений одинаковых с + и - и также парсить их гораздо проще циклом
                    else:
